...
# Optic Prophet: VSRP Scanner — Hooks Edition
# Built like I'm risking my own cash: conservative gates, loud alerts, zero fluff.
...
st.caption("We don’t predict price. We harvest **distortion** (IV vs RV) and **skew**. Every ARM is sniper‑grade or it’s a KILL.")
...
with st.sidebar:
    ...
    use_nq = st.checkbox("Use NQ/MNQ trigger overlay", value=True)
    st.caption("Uses NQ futures (NQ=F) 5‑minute trend + realized vol to avoid trading into macro haymakers.")
    st.divider()
...
def _append_metric_row(row: dict):
    ...
    _load_metrics.clear()  # invalidate cache
...
# ---------------------------- MNQ/NQ REGIME OVERLAY ---------------------------- #
@st.cache_data(show_spinner=False, ttl=180)
def _nq_regime():
    # yfinance futures symbol for Nasdaq-100: NQ=F ; if it fails, fall back to ^NDX
    try:
    ...
    hot = intraday_rv > 0.015  # >1.5% daily sigma ~ spicy tape
    regime = "OK" if (trend=="risk_on" and not hot) else ("Caution" if not hot else "Volatile")
    return {
        "status": regime, "trend": trend, "rv_5m": intraday_rv, "label": label
    }
...
def _collect_snapshot(tkr: str, target_dte: int) -> Optional[Tuple[Signal, pd.DataFrame, pd.DataFrame, float, float]]:
    try:
        ...
        e_days = None
        if edate: e_days = (edate - today).days
...
def _scan_watchlist(tickers: List[str], target_dte: int, ivrv_gate: float, skew_gate: float, ivr_credit_guard: float, width_cap: float):
    ...
    for (sig,calls,puts,spot,T), z in zip(pods, rr_z):
        ...
        # Time gate + IV Rank from our history
        ...
        rows.append({
            ...
            "earnings_trap":earnings_trap, "play":sig.play,
            "spread": spread.__dict__ if spread else None
        })
...
# ---------------------------- RUN SCAN ---------------------------- #
if run_btn:
    with st.spinner("Scanning for mispriced volatility & skew distortion…"):
        df = _scan_watchlist(tickers, target_dte, ivrv_gate, skew_gate, ivr_credit_guard, width_cap)

    if df.empty:
        st.warning("No data. Bad ticker list, network issue, or market closed too far out.")
    else:
        # Pretty table
        show_cols = ["ticker","price","expiry","dte","ivrv_ratio","rr25","skew_z","iv_rank","ivr_delta","vega_flip","momentum","time_gate","earnings_trap","play"]
        ...
        # Downloads
        st.download_button("⬇️ Download scan as CSV", df.to_csv(index=False).encode(), file_name="vsrp_scan.csv", mime="text/csv", use_container_width=True)
...
with st.expander("Log a trade (R-based)"):
    ...
    t_time = colC.datetime_input("Execution time (UTC)", value=datetime.utcnow())
    add_btn = colD.button("Add to log")
    if add_btn:
        try:
            df = pd.read_csv(TRADE_LOG_CSV, parse_dates=["timestamp"])
        except Exception:
            df = pd.DataFrame(columns=["timestamp","ticker","R"])
        df = pd.concat([df, pd.DataFrame([{"timestamp":t_time, "ticker":t_ticker.upper(), "R":t_R}])], ignore_index=True)
        df.to_csv(TRADE_LOG_CSV, index=False)
        st.success("Logged.")
        st.download_button("Download log CSV", df.to_csv(index=False).encode(), file_name="vsrp_trade_log.csv", mime="text/csv")
...
with st.expander("Show time‑of‑day heatmap data"):
    try:
        ...
        agg = log.groupby(["dow","hour"]).agg(
            ...
            avg_R=("R","mean"),
            n=("R","count")
        ).reset_index().sort_values(["dow","hour"])
...
# ---------------------------- FOOTER ---------------------------- #
st.caption("This app is for **process**. It hunts IV/RV distortion and skew like a machine and says 'KILL' often. Flat = permission to reload, not a license to tilt.")
